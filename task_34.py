...
poem = input().lower().replace('-', '').split()
vowels = 'аеёиоуыэюя'

if ''.join(poem).isalpha():
    syllables = [sum(list(map(lambda x: phrase.count(x), vowels))) for phrase in poem]

    if len(set(syllables)) == 1 and 0 not in syllables:
        print('Парам пам-пам')
